...
export default function TemplatesClient() {
    ...
    // Wrapper to handle navigation/preview for the card
    const TemplateCardWrapper = ({ template }: { template: Template }) => {
        ...
        const CardContent = (
            <div className="w-[280px] h-[340px] flex-shrink-0 snap-start">
                <TemplateCard
                    template={template as any}
                    onClick={() => !canEdit && handlePreview(template)}
                    isSelected={false}
                />
            </div>
        );
                                            ...
                                            <div className="relative group/carousel">
                                                <div className="flex overflow-x-auto gap-6 py-6 -mx-2 px-2 snap-x scrollbar-thin scrollbar-thumb-gray-200 scrollbar-track-transparent">
                                                    {group.items.map(template => (
                                                        <TemplateCardWrapper key={template.id} template={template} />
                                                    ))}
